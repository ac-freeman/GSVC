...
class SimpleTrainer2d:
    ...
    def __init__(
        ...
        self.pretrained_dict=None
        self.model_dict=None
        if model_name == "GaussianImage_Cholesky":
            from gaussianimage_cholesky_r3 import GaussianImage_Cholesky
        ...
        if Trained_Model is not None:
            ...
            model_dict = self.gaussian_model.state_dict()
            self.pretrained_dict = {k: v for k, v in checkpoint.items() if k in model_dict}
    ...
    def train(self,frame,ispos):     
        ...
        for iter in range(1, int(self.iterations)+1):
            loss, psnr = self.gaussian_model.train_iter(self.gt_image,iter,start_adaptivecontrol,strat_iter_adaptive_control,self.pretrained_dict)
        ...
        Gscheduler=self.gaussian_model.scheduler.state_dict()
        filtered_Gmodel = {
        ...
        return psnr_value, ms_ssim_value, end_time, test_end_time, 1/test_end_time, filtered_Gmodel, img, num_gaussian_points, loss,Gscheduler
        ...
        if frame_num ==1 or frame_num%50==0:
            if iswarmup:
                trainer = SimpleTrainer2d(image=downsample_image(video_frames[i],4),frame_num=frame_num,savdir=savdir,loss_type=loss_type, num_points=args.num_points, 
                    iterations=1000, model_name=args.model_name, args=args, model_path=None,Trained_Model=None,isdensity=False,removal_rate=removal_rate,Gscheduler=None)
                ...
                trainer = SimpleTrainer2d(image=downsample_image(video_frames[i],2),frame_num=frame_num,savdir=savdir,loss_type=loss_type, num_points=num_gaussian_points, 
                    iterations=1000, model_name=args.model_name, args=args, model_path=None,Trained_Model=Gmodel,isdensity=False,removal_rate=removal_rate,Gscheduler=None)
                _, _, _, _, _, Gmodel, _, num_gaussian_points, _ = trainer.train(i,ispos)
                trainer = SimpleTrainer2d(image=video_frames[i],frame_num=frame_num,savdir=savdir,loss_type=loss_type, num_points=num_gaussian_points, 
                    iterations=args.iterations, model_name=args.model_name, args=args, model_path=None,Trained_Model=Gmodel,isdensity=is_ad,removal_rate=removal_rate,Gscheduler=None)
            else:
                trainer = SimpleTrainer2d(image=video_frames[i],frame_num=frame_num,savdir=savdir,loss_type=loss_type, num_points=args.num_points, 
                    iterations=args.iterations, model_name=args.model_name, args=args, model_path=None,Trained_Model=None,isdensity=is_ad,removal_rate=removal_rate,Gscheduler=None)
        else:
            trainer = SimpleTrainer2d(image=video_frames[i],frame_num=frame_num,savdir=savdir,loss_type=loss_type, num_points=num_gaussian_points, 
                iterations=args.iterations, model_name=args.model_name, args=args, model_path=None,Trained_Model=Gmodel,isdensity=False,removal_rate=removal_rate,Gscheduler=Gscheduler)
        psnr, ms_ssim, training_time, eval_time, eval_fps, Gmodel, img, num_gaussian_points, loss, Gscheduler = trainer.train(i,ispos)
        img_list.append(img)
        psnrs.append(psnr)
