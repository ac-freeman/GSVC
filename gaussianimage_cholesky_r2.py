...
class GaussianImage_Cholesky(nn.Module):
    ...
    def train_iter(self, gt_image,iter,isdensity,strat_iter_adaptive_control,pretrained_dict):
        ...
        if (iter) % (self.densification_interval) == 0 and iter > 0 and isdensity:
            self.density_control(iter,strat_iter_adaptive_control)
        self.optimizer.step()
