...
#SBATCH --job-name=test_C_3_test    # Job name
...
savdir="result_Clip_test"
savdir_m="models_Clip_test"
is_pos=False
is_warmup=False
is_ad=True
is_clip=True
...
for dataset in "${datasets[@]}"; do
  ...
  for num_points in 30000 40000 50000 60000 70000; do
    for iterations in 100000; do
      ...
      srun python train_video2.py --loss_type $loss_type --dataset $dataset_path \
...
is_pos=True
savdir="result_Clip_Pos_test"
savdir_m="models_Clip_Pos_test"
for dataset in "${datasets[@]}"; do
  dataset_path=$(echo $dataset | cut -d' ' -f1)
  data_name=$(echo $dataset | cut -d' ' -f2)
