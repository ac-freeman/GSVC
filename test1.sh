...
#SBATCH --job-name=videogs_loss_job_1    # Job name
...
for dataset in "${datasets[@]}"; do
  ...
  # Run the training script for each dataset
    for iterations in 50000; do
      srun python train_video_pos_density.py --dataset $dataset_path --data_name $data_name --num_points $num_points --iterations $iterations
      done
    done
done