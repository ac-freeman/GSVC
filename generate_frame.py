...
with open(file_path, 'rb') as f:
    for _ in tqdm(range(total_frames), desc="Reading YUV frames"):
        ...
        # Convert YUV frame to BGR format for OpenCV
        bgr_frame = yuv420_to_rgb(yuv_frame, width, height)

        # 将当前BGR帧添加到列表中
        video_frames_rgb.append(bgr_frame)

# 现在 video_frames_rgb 中包含所有的BGR帧数据
...
if video_frames_rgb:
    ...
    cv2.imwrite("first_frame.png", first_frame)  # 使用OpenCV保存
