...
class GaussianImage_Cholesky(nn.Module):
    ...
    def density_control_Combine(self, iter,strat_iter_adaptive_control):
        iter_threshold_remove =4000  # 根据训练计划调整这个阈值
        if iter>strat_iter_adaptive_control+iter_threshold_remove:
            return
        rgb_weight = torch.norm(self.rgb_W, dim=1)
        ...
        importance = torch.min(rgb_weight_normalized, grad_xyz_normalized)
    ...
    def train_iter_Combine(self, gt_image,iter,isdensity,strat_iter_adaptive_control):
        ...
        if (iter) % (self.densification_interval) == 0 and iter > 0 and isdensity:
            self.density_control_Combine(iter,strat_iter_adaptive_control)
        self.optimizer.step()
