...
#SBATCH --job-name=test_do_1    # Job name
...
savdir="result_density_Opacity"
savdir_m="models_density_Opacity"
is_pos=False
is_warmup=False
is_ad=True
is_clip=False
...
is_pos=True
savdir="result_density_Opacity_Pos"
savdir_m="models_density_Opacity_Pos"
for dataset in "${datasets[@]}"; do
  dataset_path=$(echo $dataset | cut -d' ' -f1)
  data_name=$(echo $dataset | cut -d' ' -f2)
