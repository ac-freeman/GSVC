...
class SimpleTrainer2d:
    ...
    def train(self,frame,ispos):     
        ...
        for iter in range(1, int(self.iterations)+1):
            if self.isclip:
                loss, psnr,img = self.gaussian_model.train_iter_img_grad(self.gt_eimage,iter,self.isdensity)
            else:
                loss, psnr,img = self.gaussian_model.train_iter_img_grad(self.gt_image,iter,self.isdensity)
