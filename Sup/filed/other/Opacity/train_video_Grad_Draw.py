...
class SimpleTrainer2d:
    ...
    def __init__(
        ...
        if model_name == "GaussianImage_Cholesky":
            from gaussianimage_cholesky_draw import GaussianImage_Cholesky
            if self.isclip:
    ...
    def train(self,frame,ispos):     
        ...
        for iter in range(1, int(self.iterations)+1):
            if self.isclip:
                loss, psnr,img, grad = self.gaussian_model.train_iter_img_grad(self.gt_eimage,iter,self.isdensity)
            else:
                loss, psnr,img, grad = self.gaussian_model.train_iter_img_grad(self.gt_image,iter,self.isdensity)
            ...
            with torch.no_grad():
                ...
                if iter==1 or iter % 50 == 0:
                    ...
                    out_pos_grad =self.gaussian_model.forward_pos_grad(num_gaussian_points, grad)
        ...
        self.test(num_gaussian_points, grad)
    ...
    def test(self,num_gaussian_points, grad):
        ...
        img_pos_opacity = transform(out_pos_grad_img.float().squeeze(0))
